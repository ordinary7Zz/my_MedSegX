...
def compute_hausdorff_distance(
    ...
    for b, c in np.ndindex(batch_size, n_class):
        ...
        # ensure edges and masks are numpy arrays for numpy/scipy operations
        if isinstance(edges_gt, torch.Tensor):
            edges_gt_np = edges_gt.cpu().numpy()
        else:
            edges_gt_np = np.asarray(edges_gt)

        if isinstance(edges_pred, torch.Tensor):
            edges_pred_np = edges_pred.cpu().numpy()
        else:
            edges_pred_np = np.asarray(edges_pred)

        if not edges_gt_np.any():
            warnings.warn(f"the ground truth of class {c} is all 0, this may result in nan/inf distance.")

        if not edges_pred_np.any():
            # convert ground truth mask to numpy for distance transform
            if isinstance(y[b, c], torch.Tensor):
                y_np = y[b, c].cpu().numpy()
            else:
                y_np = np.asarray(y[b, c])
            dist_map = distance_transform_edt(1 - y_np)
            foreground_distances = dist_map[y_np == 0]
            hd[b, c] = np.percentile(foreground_distances, 95)
        else:
            distance_1 = compute_percent_hausdorff_distance(
                edges_pred_np, edges_gt_np, distance_metric, percentile, spacing_list[b]
            )
            if directed:
                hd[b, c] = distance_1
            else:
                distance_2 = compute_percent_hausdorff_distance(
                    edges_gt_np, edges_pred_np, distance_metric, percentile, spacing_list[b]
                )

...
# setup parser
parser = argparse.ArgumentParser("MedSedX tuning", add_help=False)
# model
...
def main(args):
    ...
    for task in sorted(os.listdir(data_path)):
        ...
        for dataset in sorted(os.listdir(task_path)):
            ...
            sequence = None
            if "finetune" not in os.listdir(dataset_path):
                # modality with sequences
                sequence = os.listdir(dataset_path)[0]
                dataset_path = join(dataset_path, sequence)

            train_dataset = TaskMedSegDB(join(dataset_path, "finetune"), train=True, sequence=(sequence is not None))
            ...
            if args.validation == "val":
                ...
                val_dataset = TaskMedSegDB(join(dataset_path, "inference"), train=False, sequence=(sequence is not None))
            ...
            save_dir = join(work_dir, task, dataset)
            if sequence is not None:
                # modality with sequences
                save_dir = join(save_dir, sequence)
